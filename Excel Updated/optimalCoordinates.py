...
def get_days(delta):
    ...
    if len(str(month_end))<2:
        month_end_str = '0'+str(month_end)
    else:
        month_end_str = str(month_end)

    if len(str(month_start))<2:
        month_start_str = '0'+str(month_start)
    else:
        month_start_str = str(month_start)
    
    sheet_name = f"{year-2000}{month_start_str}" if month_start_str == month_end_str else [f"{year-2000}{month_start_str}", f"{year-2000}{month_end_str}"]
...
def search_date(df, date_dict):
    start_date = pd.to_datetime(get_date_format(date_dict['year'], date_dict['start_day'], date_dict['start_month']))
    end_date = pd.to_datetime(get_date_format(date_dict['year'], date_dict['end_day'], date_dict['end_month']))
...
def write_ics(df, user_name):
    ...
    for _, row in df.iterrows():
        ...
        # This if statement exists to correct the event's date in case it has a note in the Date column
        # The Date column has comments when it's specified that the service described is refered to the year prior to the current one (i.e: 'referente a 2024')
...
def get_user_choice(folders, date_dict):
    ...
    def select_folder():
        ...
        #os.chdir(f"//192.168.9.14/e/GY/AGENDA/2024/{folder}")
        root.quit()
...
def main(delta):
    ...
    filtered_df = df[df['Client'].notna() & (df['Client'] != 'Almoço') & (df['Client'] != 'chegada')] 
    #Filter out the rows that include the words Almoço or chegada in the Client column
    # The rows with 'deslocação' need to be used to guarantee the correct days for every event if there is a scheduled service that refers to the year prior to the current one
